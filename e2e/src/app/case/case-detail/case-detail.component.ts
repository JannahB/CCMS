...
export class CaseDetailComponent implements OnInit, OnDestroy{
  ...
  caseDocumentFilingDate: Date = null;
  ...
  onUpload(event) {
    ...
    // Reset Document type and category if upload was successful
    ...
    this.caseSubscription = this.caseSvc.fetchOne(currCaseOID).subscribe(
      ...
      () => {
        ...
        this.uploadedFiles.length = 0;  // reset files list to remove from view
      }
    );
    const oUploadResp = JSON.parse(event.xhr.status);
    if (oUploadResp === 200 || oUploadResp === 201) {
  ...
  onBeforeSendFile(event) {
    ...
    if (
      ...
      if(this.caseDocumentFilingDate) {
        const filingDate = this.datePipe.transform(this.caseDocumentFilingDate, "yyyy-MM-dd HH:mm");
        event.xhr.setRequestHeader("caseDocumentFilingDate", filingDate);
      }
      this.caseDocumentFilingDate = null;
    }
  }
